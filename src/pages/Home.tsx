...
export default function Home() {
  ...
  const downloadFile = async (fileUrl: string, type: string) => {
    try {
      ...
      showToast(t('common.downloadStarted'), 'success');
    } catch (err) {
      window.open(fileUrl, '_blank');
      showToast(t('common.downloadOpened'), 'success');
    }
  };
  ...
  const downloadImage = async (imgUrl: string, index: number) => {
    try {
      ...
      link.click();
      document.body.removeChild(link);
      showToast(t('common.imageDownloaded'), 'success');
    } catch (err) {
      window.open(imgUrl, '_blank');
      showToast(t('common.imageOpened'), 'success');
    }
  };
  ...
  const downloadAllAsZip = async () => {
    ...
    try {
      setLoading(true);
      showToast(t('common.preparingZip'), 'success');
      const zip = new JSZip();
      ...
      link.click();
      document.body.removeChild(link);
      showToast(t('common.zipStarted'), 'success');
    } catch (err) {
      showToast(t('common.zipFailed'), 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          {result && (
            ...
            >
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 className="text-xl font-black text-gray-900">{t('common.downloadResult')}</h2>
                <button 
              ...
              <div className="p-4 space-y-4">
                    ...
                    <div className="relative w-full h-full flex items-center justify-center">
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md px-4 py-2 rounded-2xl text-[10px] text-white font-black flex flex-col items-center space-y-1 shadow-2xl border border-white/10">
                        ...
                        <div className="flex items-center space-x-1 text-green-400">
                          ...
                          <span className="uppercase tracking-tighter">{t('common.tapToSave')}</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                ...
                <div className="flex flex-col space-y-2">
                  {(!result.images || result.images.length === 0) && (
                    <>
                      ...
                      >
                        ...
                        <span>{t('common.downloadNoWm')}</span>
                      </button>
                      <button
                      ...
                      >
                        <i className="fas fa-video"></i>
                        <span>{t('common.downloadWithWm')}</span>
                      </button>
                    </>
                  )}
                  ...
                  >
                    ...
                    <span>{t('common.downloadMusic')}</span>
                  </button>
                ...
                <div className="space-y-3 pt-2">
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Share to Social Media</p>
        ...
        <div className="pt-24 space-y-20 text-left">
          <section className="space-y-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-600 font-bold max-w-2xl mx-auto">{t('common.howToUseSub')}</p>
            </div>
                ...
                >
                  ...
                  <div className="space-y-4 relative z-10">
                    <h3 className="text-xl font-black text-gray-900">
                      {step === 1 ? t('common.step1Title') : step === 2 ? t('common.step2Title') : t('common.step3Title')}
                    </h3>
