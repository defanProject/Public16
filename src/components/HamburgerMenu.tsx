...
export default function HamburgerMenu({ isOpen, onClose }: HamburgerMenuProps) {
  ...
  const menuItems = [
    ...
    { icon: Flame, label: t('trending.title'), path: `/${lang || 'en'}/trending` },
    ...
    { icon: Server, label: t('status.title'), path: `/${lang || 'en'}/status` },
  ];
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 top-0 bg-white shadow-2xl z-[101] flex flex-col max-h-[90vh] overflow-y-auto rounded-b-[3rem]"
          >
            ...
            <div className="p-6 border-t border-gray-100 text-center">
              <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">SnapTok.lol v2.0</p>
            </div>
          </motion.div>
        </>
      )}
